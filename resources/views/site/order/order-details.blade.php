        ...
        <div class="flex justify-between">
            <div>
                <div class="flex items-center">
                    <span class="lg:mt-0 mt-1  ltr:mr-2 rtl:ml-2">
            ...
            <div class="lg:block hidden">
                <a href="{{ route('site.order') }}" class="flex relative mt-2 arrow-hover font-medium dm-sans text-gray-10 text-base ltr:pl-4 rtl:pr-4 rounded-sm">
                    <svg class="mt-2 absolute ltr:mr-3 rtl:ml-3 neg-transition-scale" width="15" height="10" viewBox="0 0 15 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="dm-sans font-medium ltr:ml-4 rtl:mr-4">{{ __('Back') }}</span>
                </a>
            </div>
        </div>
            ...
            <div class="xl:w-1/5 w-full">
                <div class="flex flex-row xl:flex-col justify-between">
                    <div>
                        ...
                        <a class="border border-black py-1 px-2 mt-2 inline-block" {{ json_decode(preference('invoice'))?->general->pdf_view == 'new_tap' ? 'target="_blank"' : '' }} href="{{ route('site.invoice.print', ['id' => $order->id, 'type' => 'print']) }}">
                            <span class="pdf-inv-btn">{{ __('PDF') }} {{ __('Invoice') }}</span>
                        </a>
                    </div>
                    <div class="xl:mt-10">
                        <p class="uppercase roboto-medium font-medium xl:text-lg text-15 text-gray-10 ltr:lg:text-left ltr:text-right rtl:text-left rtl:lg:text-right">{{ __('Status') }}</p>
                        <h3 class="uppercase dm-bold font-bold text-gray-12 2xl:text-28 xl:text-2xl text-xl ltr:xl:text-left ltr:text-right rtl:text-left rtl:xl:text-right">{{ $order->orderStatus->name }}</h3>
                    </div>
                </div>
            ...
            <div class="grid 2xl:grid-cols-3 xl:w-4/5 w-full lg:grid-cols-2 grid-cols-1 lg:gap-50p gap-5 mt-50p xl:mt-0">
                <div class="xl:pl-30p pl-5 pr-3 border border-gray-2 pb-6">
                    ...
                    <div class="flex flex-wrap gap-2 items-center mt-11 lg:mb-38p mb-7">
                        <p class="roboto-medium font-medium text-gray-10 capitalize text-base">{{ __('status') }}:
                         ...
                         <span class="{{ $order->payment_status == 'Paid' ? 'bg-green-2 text-green-1' : 'bg-pinks-2 text-reds-3' }}  px-3 text-lg w-max py-2 rounded-sm dm-sans font-medium ltr:ml-2 ltr:mr-1 rtl:mr-2 rtl:ml-1">{{$order->payment_status }}</span>
                            @if(optional($order->paymentMethod)->status == 'pending' && optional($order->orderStatus)->slug != 'cancelled' && optional($order->paymentMethod)->gateway != 'CashOnDelivery' && $order->payment_status != 'Partial')
                ...
                <div class="xl:px-30p px-5 border border-gray-2 pb-6">
                    ...
                    <div>
                        ...
                        <p class="mt-2 roboto-medium font-medium text-gray-10 text-sm break-all ltr:pr-5 rtl:pl-5">{{ $order->note }}</p>
                    </div>
                    @endif
                </div>
                <div class="xl:px-30p px-5 border border-gray-2 pb-6">
            ...
            <table class="w-full bg-white overflow-hidden">
                <thead>
                    ...
                    <tr class="bg-gray-11 border border-gray-2 dark:bg-gray-2 ltr:text-left rtl:text-right">
                        <th class="pl-10 py-4 w-532p text-gray-12 dm-sans font-medium xl:text-xl md:text-base dark:text-gray-2 ltr:pl-10 rtl:pr-10">
                            {{ __('Products') }}
                        </th>
                ...
                <tbody class="border border-gray-2">
                    @foreach ($detailGroups as $group)
                        ...
                        <td colspan="5" class="mt-7 pb-8 roboto-medium font-medium text-gray-10 flex items-center dark:text-gray-2 ltr:pl-10 rtl:pr-10">{{ __('Seller') }}: {{ optional($detail->vendor)->name }}
                        </td>
                        </tr>
                        @endif
                        @endforeach
                            ...
                            <tr>
                                <td class="w-532p pb-8">
                                    ...
                                    $purchaseNote = $detail->productMeta->where('key', 'meta_purchase_note')->first();
                                        @endphp
                                    <div class="justify-left w-max items-center flex ltr:pl-10 rtl:pr-10">
                                        ...
                                        <div class="flex flex-col ltr:ml-8 rtl:mr-8">
                                    ...
                                    @if (!empty($purchaseNote) && !empty($purchaseNote->value) && $purchaseNote->value != '')
                                        <span class="dm-sans font-medium text-gray-12 whitespace-normal flex items-center dark:text-gray-2 w-532p ltr:pl-10 rtl:pr-10" title="{{ $purchaseNote->value }}">
                                ...
                                <td class="w-max pb-8">
                                  <div class="flex justify-left items-center">
                                        ...
                                        @if ($detail->isRefundable() && preference('order_refund'))
                                            <a class="text-gray-10 text-sm roboto-medium font-medium mr-5 w-24 bg-gray-11 py-1.5 rounded-sm px-2.5 flex ltr:ml-6 rtl:mr-6" href="{{ route('site.createRefundRequest', ['order_id' => $order->id, 'product_id' => $detail->product_id]) }}">
                                                <svg class=" ltr:mr-2.5 rtl:ml-2.5" xmlns="http://www.w3.org/2000/svg" width="14" height="16" viewBox="0 0 14 16" fill="none">
                    ...
                    <div>
                        <div class="flex justify-between items-center mx-4 mb-4">
                            <div class="flex justify-center items-center mt-4">
                                <span class="w-12 h-12 p-1.5 flex items-center bg-gray-17 rounded ltr:mr-3 rtl:ml-3">
                                ...
                                <div>
                                    ...
                                    <p class="dm-sans mt-5p font-medium text-xss text-gray-12">
                                        <span class="ltr:mr-1 rtl:ml-1">{{ formatNumber($detail->price, optional($order->currency)->symbol) }}</span>
                                        <span class="ltr:mr-1 rtl:ml-1">x {{ formatCurrencyAmount($detail->quantity) }} </span>
                            ...
                            <div>
                                <a href="{{ route('site.createRefundRequest', ['order_id' => $order->id, 'product_id' => $detail->product_id]) }}"
                                    ...
                                    <svg class="ltr:mr-1 rtl:ml-1 mt-0.5" xmlns="http://www.w3.org/2000/svg" width="13" height="14"
