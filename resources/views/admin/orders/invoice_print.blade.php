...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('dist/css/pdf-invoice.min.css') }}">
</head>
        ...
        <div id="printTable">
            ...
            <div>
                <div class="invoice-side">
                    @if ($invoiceSetting?->document?->header?->is_invoice_no_show)
                        <p class="order-invoice">{{ empty($invoiceSetting?->document?->header?->invoice_label) ? __('Order Invoice') : $invoiceSetting?->document?->header?->invoice_label }}</p>
            ...
            <div>
                <table class="table">
                    <thead class="thead">
                        ...
                        @endif
                        <tr>
                            <th colspan="{{ $invoiceSetting?->document?->product_table?->is_image ? 2 : 0 }}">{{ $invoiceSetting?->document?->product_table?->product_label ?? __('Product Name') }}
                            </th>
                ...
                <table class="table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="width-80"></td>
                            <td class="footer-information currency_symbol">{{ formatNumber($order->tax_charge, optional($order->currency)->symbol) }}
                            </td>
                        </tr>
                        @if ($couponOffer > 0)
                            <tr>
                                ...
                                <td class="width-80"></td>
                                <td class="footer-information currency_symbol">{{ formatNumber($couponOffer, optional($order->currency)->symbol) }}
                                </td>
                            </tr>
                        @endif
                        @if ($order->other_discount_amount > 0)
                            <tr>
                                ...
                                <td class="width-80"></td>
                                <td class="footer-information currency_symbol">{{ formatNumber($order->other_discount_amount, optional($order->currency)->symbol) }}
                                </td>
                            </tr>
                        @endif
                        <tr>
            ...
            @if ($invoiceSetting?->document?->footer?->is_footer)
                @if ($invoiceSetting?->document?->footer?->is_main_footer)
                    <p class="keep-in-touch" style="color: {{ $invoiceSetting?->document?->footer?->main_footer?->text_color }}; text-align: {{ $invoiceSetting?->document->footer?->main_footer?->align }};">{{ empty($invoiceSetting?->document?->footer?->main_footer?->label) ? __('Keep in touch') : $invoiceSetting?->document?->footer?->main_footer?->label }}</p>
                    <p class="concern-queries" style="color: {{ $invoiceSetting?->document?->footer?->main_footer?->text_color }}; text-align: {{ $invoiceSetting?->document->footer?->main_footer?->align }};">{{ $invoiceSetting?->document?->footer?->main_footer?->content }}</p>
                @endif
                ...
                @if ($invoiceSetting?->document?->footer?->is_copy_right_footer)
                    @if (!$invoiceSetting?->document?->footer?->is_main_footer)
                        <p></p>
                    @endif        
                    <p class="copy-right" style="color: {{ $invoiceSetting?->document?->footer?->copy_right_footer?->text_color }}; text-align: {{ $invoiceSetting?->document->footer?->copy_right_footer?->align }};">
...
@if ($type == 'print')
    <script src="{{ asset('dist/js/custom/site/order-invoice.min.js') }}"></script>
@endif
