    ...
    <div class="col-sm-12 list-container">
        <div class="card">
            <div class="card-header d-md-flex justify-content-between align-items-center">
                <div>
                    <h5>
                        <a href="{{ route('vendor.home') }}">{{ __('Homepages') }}</a>
                        >>
                ...
                <div class="d-flex justify-content-end align-items-center">
                    <div class="form-group d-flex mb-0">
                        <button class="btn btn-success btn-sm has-spinner-loader mb-0" id="update_page" type="submit"><i
                                class="feather icon-save ltr:me-1 rtl:ms-1"></i>{{ __('Save') }}
                        </button>
                    </div>
                    <div class="form-group d-flex mb-0">
                        <a class="btn btn-sm btn-info mb-0" href="{{ route('site.shop', ['alias' => auth()->user()?->shop()?->alias ?? 'unknown', 'homepage' => $page->slug]) }}" target="_blank"><i
                                class="feather icon-eye ltr:me-1 rtl:ms-1"></i>{{ __('Preview') }}</a>
                    </div>
                    <div class="form-group d-flex mb-0">
                        <button class="btn btn-sm btn-primary mb-0 ltr:me-0 rtl:ms-0" id="add-new-widget"><i
                                class="feather icon-plus ltr:me-1 rtl:ms-1"></i>{{ __('Add Section') }}</button>
                    </div>
                </div>
        </div>
                    ...
                    <ul id="sortable" class="selector">
                        @foreach ($page->components as $component)
                            <li class="ui-state-default" data-id="{{ $level = uniqid('level_') }}">
                                ...
                                @include('cms::vendor.edit.' . $component->layout->file)
                            </li>
                        @endforeach
                    </ul>
                </div>
...
@push('styles')
    ...
    <link href="{{ asset('Modules/CMS/css/style.min.css') }}" rel="stylesheet">
...
@section('js')
    ...
    <script>
        ...
        let request;
        let __page = {{ $page->id }};
        const __gridDeleteUrl = `{{ route('vendor.builder.delete', ['id' => $page->id]) }}`;
        const __savePageUrl = "{{ route('vendor.builder.updateAll', ['id' => $page->id]) }}";
        let popupNotification;
        let deletingSectionId;
        let deletingSection;
        const ajaxResourceUrl = `{{ route('vendor.ajaxResourceSelect') }}`;
        const queryOperations = @json($queryOperations);
    </script>

    <script src="{{ asset('dist/js/xss.min.js') }}"></script>
    <script src="{{ asset('Modules/CMS/js/builder.min.js') }}"></script>
    <script src="{{ asset('Modules/CMS/js/query.min.js') }}"></script>
@endsection
